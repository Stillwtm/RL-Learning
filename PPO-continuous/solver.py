...
class Solver(object):
    ...
    def create_env_agent(cfg, seed=1, unwrap=False):
        env = RescaleAction(gym.make(cfg['env_name']), min_action=-1.0, max_action=1.0)
        if unwrap:
            env = env.unwrapped
        agent = PPO(env, cfg)
        # 设置随机种子
    ...
    def train(env, agent, cfg, last_steps=0):
        ...
        for ep in range(cfg['train_episodes']):
            state = env.reset()[0]
            ep_reward = 0
            for i in range(cfg['ep_max_steps']):
            ...
            if ((ep + 1) % 50 == 0):
                print(f"Train episode {ep+1}/{cfg['train_episodes']}: reward: {np.mean(rewards[-50:])}")
